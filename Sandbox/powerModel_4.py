...
#      normRad.append(float(normalizedRadiation[dataIndex]))
  ...
  for dataIndex in range(0,len(normalizedRadiation)):
    ...
    if float(humidity[dataIndex]) < humidityCutoff:
      ...
      X.append([1.0,float(temp),float(pressure[dataIndex]),float(humid),float(windspeed[dataIndex]),float(winddirn[dataIndex]), \
                float(temp)*float(temp),float(press)*float(press),float(humid)*float(humid),float(temp)**3.0,float(press)**3.0,float(humid)**3.0,float(humid)**8.0])
      normRad.append(float(normalizedRadiation[dataIndex]))
  Xnp = np.matrix(X)
  Xt = np.transpose(Xnp)
